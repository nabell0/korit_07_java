...
public class AgeMain {
    ...
    // 여기에 추가적으로 메서드 정의를 합니다(아까 logint()처럼)
    public static void checkAge(int age) throws InvalidAgeException {
        if(age <0 || age > 200) {
            // 조건에 맞지 않는 경우 사용자 정의 예외 발생시킬 것
            // 예외를 발샏시킨다의 의미 -> 예외 객체를 생성한다는 의미
...
//        int age = scanner.nextInt();        // 전에는 여기에 바로 작성해서 넣었는데,
        ...
        // 예외가 발생할 가능성이 있는 부분을 try문 안에 넣어주는 것이 중요.
        try {
        ...
        }catch (InputMismatchException e) {     // Scanner 사용했는데 변수의 자료형과 일치하지 않을 때 적용. "스무살", 38.7이 동일한 예외 발생
        ...
        /*
            이상의 코드에서 정상적인 데이터 한번 / 비정상적인 데이터를 한번 실행시켜봤을 때의
