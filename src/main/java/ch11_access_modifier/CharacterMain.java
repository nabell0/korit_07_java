...
class Character{
    ...
    public Character(String name, int power, String skill) {
        ...
        this.health=100; //초기 체력(매개변수는 없고, 객체 생성시에 무조건 체력 100으로 고정된다는 의미)
        this.exp=0;
    }
...
//    }
    public void attack(){
        ...
        //획득하는 버그는 막고 싶어요. 그렇다면 위의 가이드에서 작성했떤 gainExp()는 외부에서 실행되면 안됩니다.
        this.gainExp(30);
    }
...
public class CharacterMain {
    public static void main(String[] args) {
        ...
        //객체의 필드에 값 대입
//        warrior.name ="전사";         //field가 public이기 때문에 타 클래스에서 값을 직접대입가능.
...
/*
    ...
    예를 들어 게임을 하거나 SNS를 한다고 쳤을 때 닉네임의 경우는 누구나 볼 수 있음.
    그러다면 닉네임 필드는 public으로 설정.
    그렇기 때문에 getter를 통한 닉네임의 호출은 필요가 없어서 이상의 코드에서 getName() 메서드는 쓸모가없음.

    우리는 어떤 클래스/필드/메서드를 어떤 접근지정자를 설정해야할지도 고민.

    하지만 현재 수업 상황에서는 웬만하면 필드는 private으로 설정, getter/ setter를 통한 우회 설정 및 조회를 목표.
 */