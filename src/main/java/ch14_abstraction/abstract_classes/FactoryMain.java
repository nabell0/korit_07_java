...
public class FactoryMain {
    public static void main(String[] args) {
        ...
        };// ;를 꼭 명시해야하는데 이 전체 코드라인이 메서드를 정의하는 것이 아니라 객체를 생성한 것이기때문에.
        factory1.setName("가전 제품 공장");
        factory1.showInfo();
        /*
            이상의 개념은 익명 클래스로, Java1.1-> 근데 현재는 추후 학습할 람다식(Lambda Expression)으로
            대체되는 경우가 많습니다.(근데 이게 JavaScript에도 동일하게 적용)

            재사용하지 않고 한 번만, 즉 이 경우에는 FactoryMain에서만 쓰고 치울 예정이면 사용하기 좋음
        ...
        tabletFactory1.setName("구글 태플릿 공장");
