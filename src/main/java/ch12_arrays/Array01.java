...
public class Array01 {
    public static void main(String[] args) {
        ...
        // 1 2 3 4 5 로 출력될 수 있도록 수정.
        for(i = 0; i < 5; i++){                     // 5가 하드코딩 되어있음. ->literal을 그대로 씀.
            System.out.print(arr01[i] + " ");       // i가 어디에 적용되어야하는지 감안할 필요있음.
        }
        System.out.println();
        for(i = 0 ; i < arr01.length ; i++){        //arr01.length 형태로 배열 내부의 데이터 개수가 바뀌더라도 문제없음.
            System.out.println(arr01[i]+" / ");
        }


        // 근데 우리가 필드를 참조할 때 객체명.필드 형태로 참조하였음.
        // bank1.accountHolder와 같은 방식으로 참조 가능했었다.
        // 그리고 field를 직접 참조했다는 말은 length 필드는 public 이라고 볼 수 있음.

        //그렇다면 배열은 객체인가 ->완전히 그렇다고는 x / 하지만 field로 length를 가진다.
        //빈 배열 생성
        int[] arr02 = new int[10]; //10개짜리 방을 가진 비어있는 배열을 생성.

        // 이상의 모든 것을 종합했을 때,
        // 배열 내부의 방의 개수 / 주소지의 한계값 -1을 우리가 항상 알고 있는 것은 아니기 때문.
        ...
        // 그럼 우리는 배열에 값을 대입할 수가 있씁니다.
        // arr02는 현재 10개짜리 방을 지닌 배열에 해당하고, 내부에는 아무런 값이 없음.
        for( i = 0 ; i < arr02.length ; i++){
            System.out.println(arr02[i]);

        }
        // arr02에 21, 22, 23, 24, 25, ...., 30을 대입하고
        // 21/22/23/24/..../30이라고 출력될 수 있또록 코드 작성.
        for(i= 0 ; i < arr02.length ; i++){
            arr02[i] = 21 + i;
            System.out.print(arr02[i] + " / ");
        }

        // arr02에 현재 21, 22, 23.....,30이 대입
...
//            arr02[i] *= 2 ;
        ...
        // 그래서 다른 자료형의 배열이라도 기본적으로 element 를 추출하는 절차는 동일.
    }
}
