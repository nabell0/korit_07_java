...
public class CentralControl {
    ...
    public void  addDevice(Power device){
        ...
        /*
            ...
            현재 method의 결과값을 가지고 다시. 찍어서 그 다음 method를 호출했습니다.
            이상의 개념을 chaining method라고 해서 return값의 유형을 명확하게 알고 있어야 함.
            그 다음 어떤 method를 실행시킬 것인지를 알 수 있습니다.
         */
    }

    private  int checkEmpty(){ // 메서드인데 private 썼습니다. main에서 굳이
        // 몇 개냐 더 전자제품을 추가할 수 있는지 알 필요가 없을 것 같아서...
        ...
        /*
            ...
            시험에 자주 나옵니다.), 실패를 나타날 때 -1을 쓰는 경우가 많습니다.
            하지만 0과 너무 가까운 수이다보니 -100이든지, 아니면 return 값으로 나올 수 없을만한
            음수값을 지정하는 경우도 있는데,
            나중에 위에 읽는 addDevice() 메서드에서 if(checkEmpty() == -89234)로 쓰고 싶지 않으면
            -1 쓰는게 가장 보편적.
         */
    }
        ...
        for (Power device : deviceArray){
            if(device instanceof  AirConditioner){ // Power의 서브클래스가 잘못된 다운캐스팅을 하지 않도록 하는 조건문
                ...
                System.out.println("장치가 비어있습니다");
                                ...
                                // 추후 update를 동해서 다른 기기들을 지원할 때는 } else 사이에 else if문을 추가하면 됨.
