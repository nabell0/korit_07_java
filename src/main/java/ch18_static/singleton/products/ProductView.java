...
public class ProductView {
    ...
    // private으로 생성자를 선언   -> 객채가 하나만 생성되는걸 보장하기 위한 접근지정자이므로 매우 중요.
    private ProductView(){
    ...
    // static 메서드의 정의 -> 대부분의 경우 getInstance() -> 시험시 힌트였음. 클래스명. 메서드명() 호출
    public static ProductView getInstance(){
        if(instance == null){           // 현재 인스턴스가 없는 시점이라면 19번 코드라인이 true이므로
            instance = new ProductView();   //ProductView() 생성자를 호출하여 intance에 대입.
        }
        return  instance;                   // 이미 생성되었으면 바로 22번 라인이 실행됨.
    }
}
